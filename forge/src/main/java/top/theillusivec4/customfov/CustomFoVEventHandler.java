...
public class CustomFoVEventHandler {
  ...
  public void onFoVModifier(EntityViewRenderEvent.FOVModifier evt) {
    FluidState fluidstate = evt.getInfo().getFluidInCamera();
  ...
  private float getNewFovModifier() {
    ...
    if (attribute != null) {
      ...
      float walkingSpeed = player.abilities.getWalkingSpeed();
    ...
    if (player.abilities.getWalkingSpeed() == 0.0F || Float.isNaN(modifier) || Float
        .isInfinite(modifier)) {
      modifier = 1.0F;
    }
